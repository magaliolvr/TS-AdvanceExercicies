...
const Dashboard: React.FC = () => {
	const { tasks, filteredTasks, stats, loading, error, setFilters } =
		useTaskContext();
	...
	useEffect(() => {
		...
		const upcoming = tasks
			.filter((task) => {
				if (!task.dueDate || task.status === TASK_STATUSES.COMPLETED)
					return false;
			...
			<div className="quick-actions">
				...
				<div className="action-buttons">
					<button
						...
						onClick={() =>
							handleQuickFilter({ status: TASK_STATUSES.PENDING })
						}>
						<Clock size={20} />
						View Pending
					</button>
					<button
						className="action-btn"
						onClick={() =>
							handleQuickFilter({ priority: TASK_PRIORITIES.HIGH })
						}>
					...
					<button
						className="action-btn"
						onClick={() =>
							handleQuickFilter({ status: TASK_STATUSES.COMPLETED })
						}>
						<CheckCircle size={20} />
						Completed
					</button>
								...
								<div key={task.id} className="task-item">
									<div className="task-info">
										...
										<div className="task-meta">
											<span
												className={`task-status ${getStatusColor(
													task.status
												)}`}>
												{task.status}
											</span>
							...
							{upcomingTasks.map((task) => (
								<div key={task.id} className="task-item">
									<div className="task-info">
										...
										<div className="task-meta">
											<span
												className={`task-status ${getTaskStatusColor(task)}`}>
												{getTaskStatus(task)}
											</span>
											<span
												className={`task-priority ${getPriorityColor(
